...
class ProjectRecommendationScreen extends StatefulWidget {
  ...
  @override
  State<ProjectRecommendationScreen> createState() =>
      _ProjectRecommendationScreenState();
}

class _ProjectRecommendationScreenState
    extends State<ProjectRecommendationScreen> {
  ...
  Future<void> _generateProjects() async {
    ...
    final targetContext = company != null
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (mounted)
          setState(
            () => _projectsMarkdown =
                "ERROR_PROTOCOL_FAILURE: Remote node error ${response.statusCode}",
          );
      }
    } catch (e) {
      if (mounted)
  ...
  Future<void> _downloadPdf() async {
    ...
    if (content != null && content.isNotEmpty) {
      ...
      final success = await rs.exportPdfDirect(
    ...
    return Scaffold(
      ...
      floatingActionButton: _projectsMarkdown != null && !_isLoading
          ? FloatingActionButton.extended(
              ...
              label: Text(
                ResumeService().projectsUrl != null
                    ? "DOWNLOAD_PDF"
                    : "EXPORT_PDF",
